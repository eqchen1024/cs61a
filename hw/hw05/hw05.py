...
def move_stack(n, start, end):
    ...
    if n==1:
        ...
        other=6-start-end #the intermediate rod
        move_stack(n-1,start,other)#stage one move the start to intermediate rod
        print_move(start,end)#move the bottom big plate to the target
        move_stack(n-1,other,end)#stage two move the rest stack of plates from the intermediate rod to the target
###########
# Mobiles #
###########
...
def make_withdraw(balance, password):
    ...
    def h_withdraw(amount,attempt):
        ...
        if try_limit>0:
            ...
            return "Your account is locked. Attempts: "+str(error_pw)
    return h_withdraw
...
def make_joint(withdraw, old_password, new_password):
    ...
    """
    "*** YOUR CODE HERE ***"
    pw_list=[old_password]
    fake_draw=withdraw(0,old_password)
    if type(fake_draw) ==str:
        return fake_draw
    else:
        pw_list.append(new_password)
    def new_w(balance, password):
        nonlocal pw_list
        if password in pw_list:
            return withdraw(balance,pw_list[0])
        else:
...
def mul_interval(x, y):
    ...
    return interval(lower,upper)# mind the barrier of abstraction use the abstraction of interval!
...
def check_par():
    ...
    r2 = interval(0.5, 1) # Replace this line!
    return r1, r2

def multiple_references_explanation():
    return """The multiple reference problem. Different calcaulation would change the variation range of the interval. par2 provide a small variation in calcualtion than par1 (for the use of interval(1,1))"""
...
def quadratic(x, a, b, c):
    ...
    axis=-b/(2*a)
    opt=(4*a*c-b**2)/(4*a)
    low_dis=abs(lower_bound(x)-axis)
    up_dis=abs(upper_bound(x)-axis)
    if lower_bound(x)<=axis and upper_bound(x)>=axis:
        if a>0:
            if low_dis<=up_dis:
                return interval(opt,a*(upper_bound(x))**2+b*upper_bound(x)+c)
            else:
                return interval(opt,a*(lower_bound(x))**2+b*lower_bound(x)+c)
        else:
            if low_dis<=up_dis:
                return interval(a*(upper_bound(x))**2+b*upper_bound(x)+c,opt)
            else:
                return interval(a*(lower_bound(x))**2+b*lower_bound(x)+c,opt)
    else:
        if a>0:
            if low_dis<=up_dis:
                return interval(a*(lower_bound(x))**2+b*lower_bound(x)+c,a*(upper_bound(x))**2+b*upper_bound(x)+c)
            else:
                return interval(a*(upper_bound(x))**2+b*upper_bound(x)+c,*(lower_bound(x))**2+b*lower_bound(x)+c)
        else:
            if low_dis<=up_dis:
                return interval(a*(upper_bound(x))**2+b*upper_bound(x)+c,*(lower_bound(x))**2+b*lower_bound(x)+c)
            else:
