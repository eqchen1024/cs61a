...
def put_piece(board, max_rows, column, player):
    ...
    for i in range(max_rows)[::-1]:
        if get_piece(board,i,column)=="-":
            new_row=replace_elem(board[i],column,player)#修改目标行中的格子
            board=replace_elem(board,i,new_row)#把修改好的行修改到棋盘上
            row=i
            break
        else:
            continue
    ...
    if row<max_rows and row>=0:
        ...
        for i in range(max_cols):
            if get_piece(board,row,i)==player:
                ...
                if cur_continue>max_continue:
                    max_continue=cur_continue
                cur_continue=0
        if cur_continue>max_continue:
            max_continue=cur_continue
        if max_continue>=num_connect:
            return True
        else:
            return False
...
def check_win(board, max_rows, max_cols, num_connect, row, col, player):
    ...
    col_win=check_win_column(board, max_rows, max_cols, num_connect,
                                      col, player)
    if diagonal_win+row_win+col_win>0:
        return True
    else:
        return False
###############################################################
