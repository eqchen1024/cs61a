...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.can_contain(insect) and self.ant.ant is None:
                    self.ant.ant=insect
                elif self.ant.can_contain(insect)==False and insect.can_contain(self.ant):
                    temp=self.ant
                    self.ant=insect
                    self.ant.ant=temp
                else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            "*** YOUR CODE HERE ***"
            if isinstance(insect,QueenAnt) and insect.is_queen==True:
                return
            else:
                self.ant=None
                return
...
class ThrowerAnt(Ant):
    ...
    min_range=0
    max_range=float('inf')
    def nearest_bee(self, hive,min_range,max_range):
        ...
        target=self.place
        while target != hive:
            if len(target.bees) !=0 and dis_count>=min_range and dis_count<=max_range:
    ...
    def action(self, colony):
        ...
        self.throw_at(self.nearest_bee(colony.hive,self.min_range,self.max_range))
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 11
        "*** YOUR CODE HERE ***"
        Place.add_insect(self,insect) # call the add_insect in Place, otherwise this is a recursive call
...
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.can_contain==True and ant.can_contain==False and self.ant==None:
            self.ant=ant
        # END Problem 9
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        for bee in bees_copy:
            bee.reduce_armor(self.damage)
        if self.ant is not None: #remind to add the behavior of body guard
            return self.ant.action(colony)
        # END Problem 10
...
# END Problem 13
    ...
    def __init__(self):
        ...
        if QueenAnt.true_queen_born==False:
            self.is_queen=True
            QueenAnt.true_queen_born=True
            self.doubled=[]
        else:
            self.is_queen=False
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_queen==False:
            ...
            while buff_place is not None:
                if buff_place.ant is not None and buff_place.ant not in self.doubled:
                    buff_place.ant.damage*=2
                    self.doubled.append(buff_place.ant)
                if isinstance(buff_place.ant,BodyguardAnt) and buff_place.ant.ant is not None and buff_place.ant.ant not in self.doubled:
                    buff_place.ant.ant.damage*=2
                    self.doubled.append(buff_place.ant.ant)
...
def apply_effect(effect, bee, duration):
    ...
    affect_action=effect(bee.action)
    def new_action(colony):
        nonlocal duration
        if duration>0:
            affect_action(colony)
            duration-=1
        else:
